...
case class TileGrid(elements: Map[Position, PositionedTile]) {
  ...
  def place(placements: NonEmptyList[PositionedTile]): PlacementResult[TileGrid] = {
    ...
    (overlapping, emptyAtOrigin, duplicatePlacement).tupled.andThen { _ =>
      ...
      val allLines: Set[NonEmptyList[PositionedTile]] =
        placements.toList.flatMap { placement =>
            val left = neighbours(result, placement.position, Direction.left)
            val right = neighbours(result, placement.position, Direction.right)
            val above = neighbours(result, placement.position, Direction.above)
            val below = neighbours(result, placement.position, Direction.below)
